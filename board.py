...
class Board:
    def __init__(self, width, height):
        ...
        self.cell_states = [[0] * width for _ in range(height)]
        self.cell_items = [[None] * width for _ in range(height)]
        self.cell_enemies = [[None] * width for _ in range(height)]
        self.left = 0
        self.top = 0
        self.cell_width = 45
        self.cell_height = 45
        self.default_cell_image = pygame.transform.scale(load_image('einschlief.png'),
                                                         (self.cell_width, self.cell_height))
        self.activated_cell_image = pygame.transform.scale(load_image('aufgeräumt.png'),
                                                           (self.cell_width, self.cell_height))
        self.blocked_cell_image = pygame.transform.scale(load_image('extra-depth.png'),
                                                         (self.cell_width, self.cell_height))
        ...
        self.initialize_board()  # Initialize board upon instantiation
    ...
    def render(self, screen, offset_x, offset_y):
        for i in range(self.height):
            for j in range(self.width):
                if j == 0 or j == self.width - 1 or i == self.height - 1 or (
                        i == 0 and (j == 0 or j == self.width - 1)):
                    screen.blit(self.blocked_cell_image,
                                (offset_x + j * self.cell_width, offset_y + i * self.cell_height))
                elif i == 0:
                    continue
                elif self.cell_states[i][j] == 1:
                    screen.blit(self.activated_cell_image,
                                (offset_x + j * self.cell_width, offset_y + i * self.cell_height))
                else:
                    screen.blit(self.default_cell_image,
                                (offset_x + j * self.cell_width, offset_y + i * self.cell_height))
        ...
        for i in range(self.height):
            for j in range(self.width):
                if self.cell_items[i][j] == 1:  # Stone
                    screen.blit(
                        pygame.transform.scale(load_image('treasure_1.png'), (self.cell_width, self.cell_height)),
                        (offset_x + j * self.cell_width, offset_y + i * self.cell_height))
                elif self.cell_items[i][j] == 2:  # Chest
                    screen.blit(
                        pygame.transform.scale(load_image('treasure_2.png'), (self.cell_width, self.cell_height)),
                        (offset_x + j * self.cell_width, offset_y + i * self.cell_height))

        self.enemy_animation_counter += 1
        if self.enemy_animation_counter >= 15:  # Animation speed
            self.enemy_animation_counter = 0
    ...
    def initialize_board(self):
        for y in range(self.height):
            for x in range(self.width):
                self.cell_states[y][x] = 0
                self.cell_items[y][x] = None
                self.cell_enemies[y][x] = None
        self.place_item(1, 1, 1)
        self.place_enemy(3, 3)
