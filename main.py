...
def show_level_message(screen, level):
...
def reset_game(game):
    game.player.pos_x = start_position_x
    game.player.pos_y = start_position_y
    game.current_health = initial_health
    game.money = 0
    game.board.reset()
start_position_x = 0
start_position_y = +1
...
def draw_records_button(screen, records_open, records=None):
    ...
    text = font.render("REC", True, (255, 215, 0))
    text_rect = text.get_rect(center=button_rect.center)
    screen.blit(text, text_rect)
...
def main():
    ...
    total_money = 0  # Суммарные деньги за уровни
    level_threshold = 1000  # Начальное требуемое количество денег для первого уровня
    max_level_threshold = 15000  # Максимальное требуемое количество денег для последнего уровня
    level_message_displayed = False  # Флаг для отображения уровня
    time_played = 0  # Время игры
    ...
    while True:
        ...
        while running:
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_F11:
                        if screen.get_flags() & pygame.FULLSCREEN:
                            screen = pygame.display.set_mode(size)  # Возврат в оконный режим
                        else:
                            screen = pygame.display.set_mode(size, pygame.FULLSCREEN)  # Полноэкранный режим
            ...
            if game.current_health <= 0:
                ...
                restart, save_record = show_game_over_screen(screen, size, total_money, time_played)

                if save_record:
                    db.add_record(total_money, int(time_played))
                    records = db.get_top_records()  # Обновляем список рекордов

                if restart:
                    level = 1  # Обнуляем уровень
                    total_money = 0  # Обнуляем суммарные деньги
                    level_threshold = 1000  # Сброс порога на следующий уровень
                    time_played = 0  # Обнуляем время
                    reset_game(game)
            ...
            if game.money >= level_threshold and not level_message_displayed:
                total_money += game.money  # Добавляем текущие деньги к суммарным
                game.money = 0  # Обнуляем текущие деньги
                level_message_displayed = True

                # Пауза перед переходом на следующий уровень
                pygame.time.delay(1000)  # Задержка перед новым уровнем
                show_level_message(screen, level)  # Отображаем уровень

                level += 1  # Переходим на следующий уровень
                if level_threshold < max_level_threshold:
                    level_threshold += 500  # Увеличиваем порог на 500 до 15000

                level_message_displayed = False  # Сбрасываем флаг после отображения

            screen.fill((2, 137, 255))

            offset_x = width // 2 - game.board.cell_width * game.player.pos_x
            offset_y = height // 2 - game.board.cell_height * (game.player.pos_y + 1)
            ...
            draw_money_counter(screen, total_money + game.money)  # Отображаем суммарные деньги
